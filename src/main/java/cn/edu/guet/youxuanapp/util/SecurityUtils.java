package cn.edu.guet.youxuanapp.util;
...
public class SecurityUtils {
    ...
    public static JwtAuthenticationToken login(HttpServletRequest request, String username, String password, AuthenticationManager authenticationManager) {
        ...
        // 生成令牌并返回给客户端
        token.setToken(cn.edu.guet.youxuanapp.util.JwtTokenUtils.generateToken(authentication));
        return token;
    }
    ...
    public static void checkAuthentication(HttpServletRequest request) {
        // 获取令牌并根据令牌获取登录认证信息
        Authentication authentication = cn.edu.guet.youxuanapp.util.JwtTokenUtils.getAuthenticationeFromToken(request);
        // 设置登录认证信息到上下文
        SecurityContextHolder.getContext().setAuthentication(authentication);
    }
